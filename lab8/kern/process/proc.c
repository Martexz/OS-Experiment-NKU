...
load_icode(int fd, int argc, char **kargv)
{
    ...
    for (; phnum < elf->e_phnum; phnum++)
    {
        ...
        //(3.4) call mm_map fun to setup the new vma ( ph->p_va, ph->p_memsz)
        ...
        //(3.5) alloc memory, and  copy the contents of every program section (from, from+end) to process's memory (la, la+end)
        end = ph->p_va + ph->p_filesz;
        //(3.5.1) copy TEXT/DATA section of bianry program
        while (start < end)
        {
